...
class TestCloudWrappersModule(unittest.TestCase):
    ...
    def test_upload_to_blob_success(self, mock_blob_service_client):
        ...
        mock_blob_client.url = f"https://{self.storage_account}.blob.core.windows.net/{self.container_name}/{self.blob_name}"
    ...
    def test_delete_blob_success(self, mock_blob_service_client):
        ...
        result = cloud_wrappers.delete_blob(
            self.storage_account,
            self.storage_key,
            self.container_name,
            self.blob_name
    ...
    def test_upload_to_gcs_success(self, mock_storage_client):
        ...
        result = cloud_wrappers.upload_to_gcs(
            self.test_file_path,
            self.bucket_name,
            "test-audio.wav"
        )
    ...
    def test_transcribe_from_gcs(self, mock_speech_client):
        ...
        result = cloud_wrappers.transcribe_from_gcs(
            gcs_uri,
            language="en-US",
            enable_diarization=True,
            max_speakers=2
    ...
    def test_delete_from_gcs_success(self, mock_storage_client):
        ...
        result = cloud_wrappers.delete_from_gcs(
            self.bucket_name,
            "test-audio.wav"
        )
