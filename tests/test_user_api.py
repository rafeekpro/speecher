...
class TestUserManagementAPI:
    ...
    def authenticated_client(self, client: TestClient) -> tuple[TestClient, Dict[str, str]]:
        ...
        # Register user
        register_data = {"email": "auth_user@example.com", "password": "SecurePass123!", "full_name": "Auth User"}
        ...
        # Login
        login_data = {"email": "auth_user@example.com", "password": "SecurePass123!"}
    ...
    def test_update_profile_duplicate_email(self, client: TestClient):
        ...
        # Login as user2
        login_data = {"email": "user2@example.com", "password": "SecurePass123!"}
        login_response = client.post("/api/auth/login", json=login_data)
    ...
    def test_change_password(self, authenticated_client):
        ...
        login_data = {"email": "auth_user@example.com", "password": "NewSecurePass456!"}
        login_response = client.post("/api/auth/login", json=login_data)
    ...
    def test_api_key_authentication(self, client: TestClient):
        ...
        register_data = {"email": "apikey@example.com", "password": "SecurePass123!", "full_name": "API Key User"}
        client.post("/api/auth/register", json=register_data)

        login_data = {"email": "apikey@example.com", "password": "SecurePass123!"}
        login_response = client.post("/api/auth/login", json=login_data)
        headers = {"Authorization": f"Bearer {login_response.json()['access_token']}"}
    ...
    def test_expired_api_key(self, client: TestClient):
        ...
        # Register user and get token
        register_data = {"email": "expired@example.com", "password": "SecurePass123!", "full_name": "Expired Key User"}
        client.post("/api/auth/register", json=register_data)

        login_data = {"email": "expired@example.com", "password": "SecurePass123!"}
        login_response = client.post("/api/auth/login", json=login_data)
        headers = {"Authorization": f"Bearer {login_response.json()['access_token']}"}
