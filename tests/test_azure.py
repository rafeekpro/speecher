...
class TestAzureModule(unittest.TestCase):
    ...
    def test_upload_file_to_blob(self, mock_generate_sas, mock_blob_service_client):
        ...
        with patch('builtins.open', mock_open(read_data=b'dummy_wav_data')):
            ...
            expected_url_start = f"https://mystorageaccount.blob.core.windows.net/{self.container_name}/{expected_blob_name}?sastoken"
    ...
    def test_get_transcription_job_status(self, mock_get):
        ...
        result = azure.get_transcription_job_status(
            self.mock_subscription_key,
            self.mock_region,
            job_id
        )
        ...
        self.assertEqual(
            mock_get.call_args[1]['headers']['Ocp-Apim-Subscription-Key'], 
            self.mock_subscription_key
        )
    ...
    def test_wait_for_job_completion_success(self, mock_sleep, mock_get):
        ...
        mock_get.side_effect = [
            self.mock_transcription_response,  # Running
            self.mock_complete_response  # Succeeded
        ]
        ...
        result = azure.wait_for_job_completion(
            self.mock_subscription_key,
            self.mock_region,
            job_id,
            poll_interval=1
    ...
    def test_download_transcription_result(self, mock_get):
        ...
        result = azure.download_transcription_result(
            self.mock_subscription_key,
            result_url
        )
        ...
        mock_get.assert_called_once_with(
            result_url, 
            headers={"Ocp-Apim-Subscription-Key": self.mock_subscription_key}
        )
    ...
    def test_delete_blob_from_container(self, mock_blob_service_client):
        ...
        result = azure.delete_blob_from_container(
            self.mock_connection_string,
            self.container_name,
            blob_name
        )
    ...
    def test_calculate_service_cost(self):
        ...
        if hasattr(azure, 'calculate_service_cost'):
            ...
            expected_keys = [
                'audio_length_seconds', 'audio_size_mb', 'transcribe_cost',
                'storage_cost', 'transaction_cost', 'total_cost', 'currency'
            ]
            ...
            expected_total = (cost_info.get('transcribe_cost', 0) + 
        ...
        if hasattr(azure, 'transcribe_short_audio'):
            ...
            with patch('builtins.open', mock_open(read_data=b'dummy_wav_data')):
                ...
                mock_speech_config.assert_called_once_with(
                    subscription=self.mock_subscription_key, 
                    region=self.mock_region
                )
