...
from src.backend.streaming import WebSocketManager, StreamingTranscriber
...
class TestWebSocketMessageValidation:
    ...
    async def test_message_size_limit(self):
        ...
        oversized_message = {
            "type": "audio",
            "data": "x" * (10 * 1024 * 1024 + 1)  # 10MB + 1 byte
        }
...
class TestWebSocketErrorHandling:
    ...
    async def test_transcription_error_recovery(self):
        ...
        with patch.object(manager.transcribers[client_id], 'process_audio_chunk', 
...
class TestWebSocketRateLimiting:
    ...
    async def test_rate_limit_exceeded(self):
        ...
        for i in range(20):
            result = await manager.process_message_with_rate_limit(
                client_id, 
                {"type": "audio", "data": f"data_{i}"}
            )
