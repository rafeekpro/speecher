...
from src.backend.models import (
    ProjectDB, ProjectResponse, ProjectStatus, ProjectUpdateRequest,
    RecordingDB, RecordingResponse,
    TagDB
)
...
def create_project(user_id: str, name: str, description: Optional[str] = None, tags: List[str] = None) -> ProjectResponse:
...
def get_user_projects(user_id: str, search: Optional[str] = None, tags: Optional[List[str]] = None) -> List[ProjectResponse]:
...
def delete_project(project_id: str) -> bool:
    ...
    recordings_to_delete = [
        rid for rid, rec in recordings_db.items() 
...
def get_project_recordings(project_id: str) -> List[RecordingResponse]:
    ...
    for recording in recordings_db.values():
        if recording.project_id == project_id:
            project_recordings.append(RecordingResponse(
                id=recording.id,
                project_id=recording.project_id,
                user_id=recording.user_id,
                filename=recording.filename,
