...
def main():
    ...
    parser.add_argument("--language", default="pl-PL", help="Kod języka do transkrypcji (domyślnie: pl-PL, np. es-ES dla hiszpańskiego)")
    parser.add_argument("--max-speakers", type=int, default=5, help="Maksymalna liczba mówców do identyfikacji (domyślnie: 5)")
    parser.add_argument("--output-file", help="Ścieżka do pliku wyjściowego, gdzie zostanie zapisana transkrypcja")
    parser.add_argument("--include-timestamps", action="store_true", default=True, help="Czy dołączać znaczniki czasu do transkrypcji (domyślnie: włączone)")
    ...
    if args.show_cost and audio_length_seconds is None:
        try:
            ...
            try:
                ...
                logger.warning("Nie można określić długości pliku audio - zainstaluj bibliotekę pydub używając komendy: poetry add pydub")
                ...
                logger.warning(f"Używam domyślnej długości pliku audio: {audio_length_seconds:.2f} sekund. Użyj parametru --audio-length, aby podać dokładną długość.")
    ...
    try:
        ...
        if not transcription.process_transcription_result(transcription_data, output_file=args.output_file, include_timestamps=args.include_timestamps):
            return 1
